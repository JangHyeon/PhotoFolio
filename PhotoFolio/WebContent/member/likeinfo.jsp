...
<body>
   <div id="page">
      ...
      <center>
        ...
        <img src="<%=info.get(i).getProfileimg()%>"style="width:180px;height:180px"><br>
      ...
      <div id="memberinfo"> 
           <center>
       			<img src="<%=list.get(j).getProfileimg()%>" style="width: 50px;height: 50px" ><br>
		  	이름:<a href="likeinfo2process?id=<%=list.get(j).getId() %>"><%=list.get(j).getNickname()%>님 </a><br>
		      	<%=list.get(j).getEmail()%><br>
		    	 소개:<%=list.get(j).getMemo()%><br>
		    	 <%if(myid.equals(id)&&check.equals("checkok")){ %>
 	      <a href="likeadd?id=<%=list.get(j).getId()%>&myid=<%=id%>">관심추가</a><br>
		    </center>
		    	 <%} %>
	 	</div>
	       <%}%>
